...
Configuration::Configuration(const QList<QString>& arguments, QObject* parent): QObject(parent), address(""), port(0) {
  ...
  QCommandLineOption configFileOption("config", "Load configuration from <config>.", "config");
  ...
  QCommandLineOption addressOption(QStringList() << "a"
                                                 << "address",
                                   "The server will listen on address <address>.",
                                   "address");
  parser.addOption(addressOption);

  QCommandLineOption publicKeyOption("public-key", "The public RSA256 key file in .pem format.", "publickey");
  parser.addOption(publicKeyOption);

  QCommandLineOption issuerOption("issuer", "The jwt need to have this issuer set.", "issuer");
  parser.addOption(issuerOption);

  QCommandLineOption claimsOption("claims",
                                  "Specify a required claim. You can use this option multiple times to require multiple claims. If "
                                  "present, all required claims set in the config file are ignored",
                                  "claims");
  parser.addOption(claimsOption);

  QCommandLineOption checkOption("check", "If set the public key and issuer will be checked with the auth server.");
  parser.addOption(checkOption);

  QCommandLineOption noCheckOption(
      "no-check",
  ...
  QCommandLineOption retrieveOption("retrieve", "If set the public key and issuer will be retrieved from the auth server.");
  parser.addOption(retrieveOption);

  QCommandLineOption noRetrieveOption(
      "no-retrieve",
  ...
  QCommandLineOption authServerOption("auth-server", "The url of the auth server. This should look like \"wss://0.0.0.0:443\".", "auth-server");
  parser.addOption(authServerOption);

  QCommandLineOption storagePathOption("storage", "Scheduled plans will be stored under in <storage>", "storage");
  parser.addOption(storagePathOption);

  QCommandLineOption jobLifetimeOption("job-lifetime", "Finished jobs will be kept for this duration in seconds", "job-lifetime");
  ...
  QCommandLineOption legacySchedulerBinaryOption("legacy-scheduler-binary", "The SPA-algorithmus binary to use", "legacy-scheduler-binary");
  ...
  if(parsedConfigurationFile == "") {
    ...
    if(!found) {
      ...
      failConfiguration("No valid configuration file found. You can " + defaultConfigFilesMessage +
...
void Configuration::loadConfiguration(const QFile& file) {
  try {
    ...
    auto parseClaims = config->get_array_of<std::string>("security.claims")
                           .value_or(std::vector<std::string>(defaultRequiredClaims.begin(), defaultRequiredClaims.end()));
    ...
    auto parseLegacySchedulerAlgorithmBinary = config->get_as<std::string>("scheduler.legacy.spaAlgorithmBinary")
                                                   .value_or(defaultLegacySchedulerAlgorithmBinary);
